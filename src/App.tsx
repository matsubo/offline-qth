  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Failed to fetch data - non-critical, stats will just not display
      }
    };
    void fetchData();
  }, []);
        ...
        <main className="space-y-6">
          <div className="card-technical rounded p-4 animate-fade-in">
            <div className="flex items-center justify-center gap-3">
              ...
              <div className="text-center font-mono-data text-sm tracking-wider text-teal-300">
                {t(status)}
              </div>
            </div>
          </div>
            ...
            <div className="card-technical rounded-none animate-fade-in overflow-hidden corner-accent">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-3">
                    {location.accuracy !== null && (
                      <ResultItem
                        ...
                        value={`±${String(Math.round(location.accuracy))}m`}
                        loading={status === "status.fetching"}
                      />
                    )}
                    <ResultItem
              ...
              <div className="border-b border-teal-500/20 data-panel">
                <div className="px-5 py-3 border-l-4 border-l-teal-500 relative z-10">
                  <div className="text-[10px] font-mono-data glow-teal tracking-wider mb-2">
                    [ LOCATION DATA ]
                  </div>
              ...
              <div className="px-5 py-4 border-l-4 border-l-amber-500">
                <div className="text-[10px] font-mono-data glow-amber tracking-wider mb-3">
                  [ AMATEUR RADIO DATA ]
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <ResultItem
                    label={t("label.gridLocator")}
                    value={location.gridLocator}
                    highlight
                    loading={status === "status.fetching"}
                  />
                  <ResultItem
